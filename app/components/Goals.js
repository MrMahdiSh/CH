...
export default function Goals({ openPopUp }) {
  ...
  const toggleStatus = async (id) => {
    try {
      ...
      await axios.post(`http://127.0.0.1:8000/api/v1/goals/${id}`, updatedGoal);
      ...
      {expanded ? (
        <div>
          {goals.map((goal) => (
            <div
              key={goal.id}
              className={`p-4 mb-2 rounded-lg text-[#E0E0E0] ${goal.status ? 'bg-green-600' : 'bg-[#3A3A4F]'}`}
            >
              <p><strong>Name:</strong> {goal.description}</p>
              ...
              <div className="flex items-center mt-2">
                <input
                  ...
                  className="mr-2"
                />
                <span>{goal.status ? 'Done' : 'Pending'}</span>
              </div>
            </div>
          ))}
          <button
